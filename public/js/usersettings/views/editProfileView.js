...
define([
  ...
  //'/js/lib/bootstrap-table.js'
    ...
    render: function () {
      ...
      try {
        ...
        if(userdata.publicProfile) {        
          this.$el.find('#showProfile').prop('checked', true); //Check the box
          this.toggleAbout();
        }
        
        //Display device IP address info.
    ...
    //This function is called whenever the user checks or unchecks the 'Public Profile Page' checkbox.
    toggleAbout: function() {
      //debugger;
      try {
        log.push('Executing editProfileView.js/toggleAbout()');
        
        //'Public Profile Page' checkbox is checked.
        if($('#showProfile:checked').length) {
          this.$el.find('#aboutDiv').slideDown(); //Show About box.

          //Fill out the About textarea if there is data for it.
          if(userdata.about) {
            this.$el.find('#aboutText').val(userdata.about);
          }


        //'Public Profile Page' checkbox is unchecked.
        } else {
          this.$el.find('#aboutDiv').slideUp(); //Hide About box.
        }
        
      } catch(err) {
        debugger;
        var msg = 'Error in editProfileView.js/toggleAbout() Error: '+err.message;
        console.error(msg);
        log.push(msg);
        sendLog();
    ...
    renderDeviceInfo: function() {
      ...
      $.get('/api/userdata/'+userdata.userData, '', function(data) {
        ...
        var deviceInfo = data.geodata;
        
        thisView.$el.find('#localIp').text(deviceInfo.localIp);
        thisView.$el.find('#globalIp').text(deviceInfo.globalIp);
        
        var timestamp = new Date(deviceInfo.timestampIp);
        thisView.$el.find('#timestampIp').text(timestamp.toLocaleString());
